...
ADD COLUMN predecessor_enrollment_id UUID NULL REFERENCES program_enrollments(id);

-- Add RRH-specific move-in date
ALTER TABLE program_enrollments
ADD COLUMN residential_move_in_date DATE NULL;

-- Add household_id column for HMIS compliance
ALTER TABLE program_enrollments
ADD COLUMN household_id VARCHAR(255) NULL;

-- Add joint project grouping to link TH and RRH program components
ALTER TABLE programs
ADD COLUMN joint_project_group_code VARCHAR(50) NULL;

-- Index for efficient predecessor lookups
CREATE INDEX idx_program_enrollments_predecessor 
...
CREATE INDEX idx_programs_joint_group 
...
CREATE INDEX idx_program_enrollments_move_in_date 
